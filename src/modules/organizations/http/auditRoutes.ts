...
import type { AuditService } from '@/modules/organizations/application/AuditService.js'
  ...
  async register(app: FastifyInstance): Promise<void> {
    app.get(
      '/api/v1/audit-events',
      {
        ...
        schema: { tags: ['Audit'], summary: 'List audit events' },
      },
    ...
    app.get(
      ...
      {
        preHandler: [authenticate, requireRole('OWNER', 'ADMIN')],
        schema: { tags: ['Audit'], summary: 'Get audit event' },
      },
      async (request: FastifyRequest, reply: FastifyReply) => {

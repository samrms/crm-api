...
import type { TaskService } from '@/modules/engagement/tasks/application/TaskService.js'
...
export class TaskRoutes {
  ...
  async register(app: FastifyInstance): Promise<void> {
    ...
    app.post(
      '/api/v1/tasks',
      {
        ...
        schema: {
          tags: ['Tasks'],
          summary: 'Create task',
          body: zodToJsonSchema(createTaskSchema, { target: 'openApi3' }),
        },
      },
      async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    app.patch(
      '/api/v1/tasks/:id',
      {
        ...
        schema: {
          tags: ['Tasks'],
          summary: 'Update task',
          body: zodToJsonSchema(updateTaskSchema, { target: 'openApi3' }),
        },
      },
      async (request: FastifyRequest, reply: FastifyReply) => {
        const { id } = request.params as { id: string }
    ...
    app.post(
      ...
      {
        preHandler: [authenticate, requireRole('OWNER', 'ADMIN')],
        schema: { tags: ['Tasks'], summary: 'Complete task' },
      },
      async (request: FastifyRequest, reply: FastifyReply) => {
        const { id } = request.params as { id: string }
        const task = await this.service.update({
